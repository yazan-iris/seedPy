...
class TestSteimFrame(unittest.TestCase):
    ...
    def test_byte_order(self):
        encoder = get(EncodingFormat.STEIM_1, ByteOrder.BIG_ENDIAN)
        self.assertEqual(ByteOrder.BIG_ENDIAN, encoder.byte_order)

        encoder = get(EncodingFormat.STEIM_1, ByteOrder.LITTLE_ENDIAN)
        self.assertEqual(ByteOrder.LITTLE_ENDIAN, encoder.byte_order)

    def test_encoding_format(self):
        encoder = get(EncodingFormat.STEIM_1, byte_order=ByteOrder.BIG_ENDIAN)
        self.assertEqual(EncodingFormat.STEIM_1, encoder.encoding_format)

        encoder = get(EncodingFormat.STEIM_2, byte_order=ByteOrder.LITTLE_ENDIAN)
        self.assertEqual(EncodingFormat.STEIM_2, encoder.encoding_format)

        encoder = get(EncodingFormat.STEIM_3, byte_order=ByteOrder.LITTLE_ENDIAN)
